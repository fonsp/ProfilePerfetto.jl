...
@perfetto_view short_but_important()
```
...
the samples pile up. `@perfetto_view` and `@perfetto_open` call [`Profile.clear`](https://docs.julialang.org/en/v1/stdlib/Profile/#Profile.clear)
...
@perfetto_view my_workload()

# 3'. Profile + visualize from the REPL
@perfetto_open my_workload()
```
